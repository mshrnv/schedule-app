...
const DeleteTeacherButton = ({teacher, updateTeachers}) => {
    ...
    return (
        <>
            ...
            <Modal open={open} onClose={handleClose} style={{padding: 0}}>
                <Modal.Body>
                    <h6>Вы уверены, что хотите удалить преподавателя {teacher.fio} ?</h6>
                </Modal.Body>
                <Modal.Footer>

                    <button onClick={deleteTeacher} type="button" className="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                        Удалить
                    </button>
